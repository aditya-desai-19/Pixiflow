...
export default function SizeContainer({ onLockAspectRatio, height, width, onHeightChange, onWidthChange }: SizeContainerProps) {
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-2 my-2 relative">
        <div className="flex flex-col gap-2">
          <Label htmlFor="height">{"Height"}</Label>
          <Input id="height" type="number" min={0} value={height} onChange={(e) => onHeightChange(parseInt(e.target.value) || 0)}/>
        </div>
        <div className="flex flex-col gap-2">
          <Label htmlFor="width">{"Width"}</Label>
          <Input id="width" type="number" min={0} value={width} onChange={(e) => onWidthChange(parseInt(e.target.value) || 0)}/>
        </div>
